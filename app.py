...
with st.expander("🏢 1단계: 사업장 조직경계 AI 설정 (다중 페이지 지원)", expanded=not st.session_state["boundary_context"]):
    ...
    # 💡 [핵심 복구 1] 1단계 채팅 기능 (티키타카 수정 및 드롭다운 연동)
    if st.session_state.get("image_context_step1"):
        if user_input_1 := st.chat_input("추가 지시 (예: 부산공장을 사업장 목록에 추가해 줘)", key="chat1"):
