...
with st.expander("📂 3단계: 활동자료 증빙 판독 및 산정근거 분석 (명세서 5)", expanded=True):
    ...
    if not st.session_state["facility_list"]: st.warning("⚠️ 2단계에서 배출시설을 먼저 등록하세요.")
        ...
        # 💡 [버그 수정 완료] 3단계 채팅 에러 방지 로직!
        if st.session_state.get("current_file_bytes"):
            if user_input_3 := st.chat_input("추가 지시 (예: 단가 계산이 틀렸어. 금액 5만원 나누기 단가 1500원으로 다시 계산해줘)", key="chat3"):
                st.session_state["msg_step3"].append({"role": "user", "content": user_input_3})
                with st.chat_message("user"): st.markdown(user_input_3)
                
                with st.spinner("🤖 산정 근거 및 표 수정 중..."):
                    try:
                        api_messages = [{"role": "system", "content": sys_prompt_3}]
                        b64_image = base64.b64encode(st.session_state["current_file_bytes"]).decode('utf-8')
                        api_messages.append({
                            "role": "user", 
                            "content": [
                                {"type": "text", "text": "이전에 올렸던 영수증 문서야."}, 
                                {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{b64_image}"}}
                            ]
                        })
                        for m in st.session_state["msg_step3"]:
                            if m["role"] == "user":
                                api_messages.append({"role": "user", "content": m["content"]})
                            elif m["role"] == "assistant":
                                api_messages.append({"role": "assistant", "content": m["content"]})
                        api_messages.append({"role": "user", "content": "(지시사항을 반영해서, 산정 근거와 마크다운 표를 그리고 마지막엔 반드시 수정된 결과도 ```json 묶음으로 출력해!)"})
