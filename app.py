...
with st.expander("📂 2단계: 증빙서류 AI 사진(Vision) 판독 및 계층 맵핑", expanded=True):
    ...
    # 💡 [핵심 진화] 월별 상세 내역 표를 먼저 그리고, 합계를 도출하도록 3단계 출력 지시!
    init_prompt_2 = f"""
    이 증빙자료(여러 장일 수 있음)는 '{target_workplace}' 부서(공정)에서 발생한 자료야.
    모든 페이지의 내역을 꼼꼼히 확인하고, 아래 3단계 양식에 맞추어 반드시 출력해줘.
    
    ### 📝 [1단계] 월별/건별 상세 사용 내역 (영수증 및 고지서별 분석)
    | 월(Month) 또는 일자 | 활동 자료 명칭 (예: 전력, 무연휘발유) | 사용량 | 단위 | 비고 (고지서/영수증 특이사항) |
    |---|---|---|---|---|
    | (예: 1월) | (전력) | 1500 | kWh | - |
    | (예: 2월) | (전력) | 1600 | kWh | - |
    
    ### 📝 [2단계] [명세서 5] 배출활동별 배출량 현황 (월별 합산 결과)
    | 사업장(부서)명 | Scope 분류 | 배출활동 (에너지원) | 적용 Tier | 연간 총 사용량 (합계) | 적용 배출계수 | 총 산정 배출량 (tCO2eq) |
    |---|---|---|---|---|---|---|
    | {target_workplace} | (Scope 1/2) | (예: 전력) | (예: Tier 1) | (총합 수치) | (계수) | (계산값) |
    
    ### 📝 [3단계] DB 저장을 위한 JSON 데이터 (합산된 결과만 저장)
    (반드시 ```json 과 ``` 로 감쌀 것)
    [JSON 형식 예시]
    ```json
    {{ "사업장명": "{target_workplace}", "Scope": "Scope 2", "배출활동": "전력", "사용량": 3100, "단위": "kWh", "배출계수": 0.4781, "배출량(tCO2eq)": 1.48 }}
    ```
    
    표 아래에는 모니터링 유형 검토와 결측치(누락된 월) 존재 시 보수적계산 필요성 코멘트를 달아줘.
    """
